...
impl KGetGui {
                ...
                if let Some(download) = self.downloads.iter_mut().find(|d| d.id == id) {
                    match msg {
                        ...
                        WorkerToGuiMessage::StatusUpdate(s) => {
                            ...
                            if s.contains("Verifying") || s.contains("SHA256") || s.contains("Calculating") {
    ...
    fn extract_speed(s: &str) -> Option<String> {
        ...
        for pattern in patterns {
            if let Some(idx) = s.find(pattern) {
                let start = s[..idx].rfind(|c: char| !c.is_numeric() && c != '.' && c != ' ')
    ...
    fn extract_size(s: &str) -> Option<String> {
        if let Some(idx) = s.find("Length:") {
            let rest = &s[idx + 7..];
            let size: String = rest.chars().take_while(|c| c.is_alphanumeric() || *c == '.' || *c == ' ').collect();
    ...
    fn open_download_folder(&self, path: &str) {
        let folder_path = std::path::Path::new(path).parent()
            .map(|p| p.to_path_buf())
    ...
    fn clear_completed(&mut self) {
        self.downloads.retain(|d| {
            matches!(d.status, DownloadStatus::Downloading | DownloadStatus::Pending | DownloadStatus::Verifying)
        });
    }
    ...
    fn render_header(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            if let Some(texture) = &self.logo_texture {
                ...
                ui.label(egui::RichText::new("⬇").size(28.0).color(colors::ACCENT_GREEN));
            }
            ...
            ui.vertical(|ui| {
                ...
                ui.label(egui::RichText::new("KGet").size(20.0).strong().color(colors::TEXT_PRIMARY));
                ui.label(egui::RichText::new("Modern Download Manager").size(11.0).color(colors::TEXT_MUTED));
            });
    ...
    fn render_input_section(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Frame::new()
            ...
            .show(ui, |ui| {
                ...
                // URL Input Row
                ui.horizontal(|ui| {
                    ui.label(egui::RichText::new("🔗").size(14.0).color(colors::TEXT_SECONDARY));
                    ...
                    // Paste button
                    if ui.add(egui::Button::new("📋").min_size(egui::vec2(32.0, 28.0)))
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.add(
                        egui::Button::new(egui::RichText::new(&display_path).size(12.0).color(colors::TEXT_SECONDARY))
                            .frame(true)
                    ...
                    ui.label(egui::RichText::new("⚡ Turbo").size(12.0).color(
                        if self.is_advanced { colors::ACCENT_ORANGE } else { colors::TEXT_SECONDARY }
                    ...
                    ui.label(egui::RichText::new("🔒 Verify").size(12.0).color(
                        if self.verify_iso { colors::ACCENT_PURPLE } else { colors::TEXT_SECONDARY }
    ...
    fn truncate_path(path: &str, max_len: usize) -> String {
        if path.len() <= max_len {
            ...
            if parts.len() > 2 {
                format!("...{}{}", std::path::MAIN_SEPARATOR, parts.last().unwrap_or(&""))
            } else {
    ...
    fn render_download_item(&mut self, ui: &mut egui::Ui, download: &DownloadItem) -> Option<(u64, &'static str)> {
        ...
        egui::Frame::new()
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    // Status indicator
                    ui.label(egui::RichText::new(download.status.icon())
                        .size(16.0)
                    ...
                    let display_name = if download.filename.len() > max_name_len {
                        format!("{}...", &download.filename[..max_name_len.min(download.filename.len())])
                    } else {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        match download.status {
                            DownloadStatus::Downloading | DownloadStatus::Verifying => {
                                if ui.add(egui::Button::new("✕").min_size(egui::vec2(24.0, 24.0)))
                        ...
                        // Status label
                        ui.label(egui::RichText::new(download.status.label())
                            .size(11.0)
                            .color(download.status.color()));
                    });
                });
                ...
                if matches!(download.status, DownloadStatus::Downloading | DownloadStatus::Verifying) {
                    ...
                    if download.progress > 0.05 && download.progress < 0.95 {
                        ...
                        if shimmer_x + shimmer_width < fill_rect.right() {
                            ...
                            painter.rect_filled(shimmer_rect, 4.0, egui::Color32::from_white_alpha(40));
                        }
                    }
                    ...
                    // Stats row
                    ui.horizontal(|ui| {
                        ui.label(egui::RichText::new(format!("{:.1}%", download.progress * 100.0))
                            .size(12.0)
                            .strong()
                            .color(color));
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if download.status == DownloadStatus::Verifying {
                                ui.label(egui::RichText::new("🔒 Calculating SHA256...")
                                    .size(11.0)
                                    .color(colors::ACCENT_PURPLE));
                            } else {
                                if download.is_advanced && !download.is_torrent {
                                    ui.label(egui::RichText::new(format!("⚡ {}x", download.connections))
                                        .size(11.0)
                                ...
                                if !download.eta.is_empty() {
                                    ui.label(egui::RichText::new(format!("⏱ {}", download.eta))
                                        .size(11.0)
                                ...
                                if !download.speed.is_empty() {
                                    ui.label(egui::RichText::new(format!("↓ {}", download.speed))
                                        .size(11.0)
                ...
                if download.status == DownloadStatus::Completed {
                    ...
                    ui.horizontal(|ui| {
                        if let Some(ref sha) = download.sha256 {
                            ...
                            ui.label(egui::RichText::new(format!("✓ SHA256: {}...", short_sha))
                                .size(10.0)
                ...
                if let Some(ref error) = download.error {
                    ...
                    ui.label(egui::RichText::new(format!("⚠ {}", display_error))
                        .size(11.0)
                        .color(colors::ACCENT_RED));
                }
            });
    ...
    fn render_empty_state(&self, ui: &mut egui::Ui) {
        ui.vertical_centered(|ui| {
            ...
            ui.label(egui::RichText::new("No Downloads")
                .size(20.0)
                .color(colors::TEXT_SECONDARY));
            
            ui.add_space(8.0);
            
            ui.label(egui::RichText::new("Paste a URL above to start downloading")
                .size(13.0)
                .color(colors::TEXT_MUTED));
    ...
    fn render_protocol_badge(ui: &mut egui::Ui, icon: &str, label: &str) {
        ui.vertical(|ui| {
            ...
            ui.label(egui::RichText::new(label).size(10.0).color(colors::TEXT_MUTED));
        });
    }
    ...
    fn render_footer(&self, ui: &mut egui::Ui) {
        let active_count = self.downloads.iter()
            ...
            .count();
        let completed_count = self.downloads.iter()
            .filter(|d| d.status == DownloadStatus::Completed)
            .count();
        
        ui.horizontal(|ui| {
            // Active downloads
            let active_color = if active_count > 0 { colors::ACCENT_BLUE } else { colors::TEXT_MUTED };
            ui.label(egui::RichText::new(format!("⬇ {} active", active_count))
                .size(11.0)
                .color(active_color));
            ...
            // Completed downloads
            let completed_color = if completed_count > 0 { colors::ACCENT_GREEN } else { colors::TEXT_MUTED };
            ui.label(egui::RichText::new(format!("✓ {} completed", completed_count))
                .size(11.0)
                .color(completed_color));
            
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                // Status text
                ui.label(egui::RichText::new(&self.status_text)
                    .size(11.0)
                    .color(colors::TEXT_MUTED));
            });
        });
...
impl eframe::App for KGetGui {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::bottom("footer")
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if self.downloads.iter().any(|d| d.status == DownloadStatus::Completed) {
            ...
            .show(ctx, |ui| {
                ...
                if self.downloads.is_empty() {
                    ...
                    egui::ScrollArea::vertical()
                        ...
                        .show(ui, |ui| {
                            let downloads_snapshot: Vec<_> = self.downloads.iter().cloned().collect();
        ...
        if self.downloads.iter().any(|d| matches!(d.status, DownloadStatus::Downloading | DownloadStatus::Verifying)) {
